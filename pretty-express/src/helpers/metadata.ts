...
export function getDataFromControllerClass(
  controller: Object
): IControllerMetaData {
  try {
...
export function getDataFromAllDecoratedFunction(
  ...
  try {
    ...
    propertyNames.forEach((propertyKey) => {
      ...
      // response validation and transform data

      const resTransformSchema = Reflect.getOwnMetadata(
      ...
      const resValidTransformOptions = Reflect.getOwnMetadata(
        VALIDATOR_META_KEYS.resTransformOptions,
        prototype,
        propertyKey
      );

      const resValidateBool = Reflect.getOwnMetadata(
      ...
      if (Object.values(HTTP_METHOD).includes(httpMethod)) {
        ...
        data.push({
          ...
          resTransformAndValidationData: {
            schema: resTransformSchema,
            validatorOptions: resValidationOptions,
            transformOptions: resValidTransformOptions,
            validate : resValidateBool
          },
          parameterIndex: {
